...
class SuffixTree {
	...
	class Node {
		 ...
		 * edge is stored right in the node. [start; end) interval specifies the
		 * edge, by which the node is connected to its parent node.
		 */

		int start, end = oo, link;
		int depth, parent;
		public Map<Character, Integer> next = new HashMap<Character, Integer>(4, 0.9f);
	...
	public SuffixTree() {
		try {
			out = new PrintWriter(new FileWriter(new File("st.dot")));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void addChar(char c) throws Exception {
		...
		while (remainder > 0) {
			...
			if (!nodes[active_node].next.containsKey(active_edge())) {
				...
				if (text[nodes[next].start + active_length] == c) { // observation
																	// 1
					active_length++;
			...
			if (active_node == root && active_length > 0) { // rule 1
				...
				active_node = nodes[active_node].link > 0 ? nodes[active_node].link : root; // rule
																							// 3
		}
	}
	...
	char[] edgeCharArray(int node) {
		return Arrays.copyOfRange(text, nodes[node].start, Math.min(position +1, nodes[node].end));
	}
	
	int terminatorIndex(int node) {
		int end = Math.min(position+1, nodes[node].end);
		for(int i = nodes[node].start; i<end; i++) {
			if(StringUtils.isTerminator(text[i])) {
				return i-nodes[node].start;
			}
		}
    ...
    void printLeaves(int x) {
        if (nodes[x].next.size() == 0)
            out.println("\tnode"+x+" [label=\""+nodes[x].depth+","+x+"\",shape=circle]");
        else {
    ...
    void printInternalNodes(int x) {
        if (x != root && nodes[x].next.size() > 0)
            out.println("\tnode"+x+" [label=\""+nodes[x].depth+","+x+"\",style=filled,fillcolor=lightgrey,shape=circle,width=.07,height=.07]");
    ...
    void printEdges(int x) {
        for (int child : nodes[x].next.values()) {
            ...
            //out.println("\tnode"+x+" -> node"+child+" [label=\""+"["+nodes[child].start+","+nodes[child].end+"]\",weight=3]");
            printEdges(child);
        }
    }
